...
import com.home.pay3.WXPayConstants.SignType;

public class WXPay {
	
	private static final String PADDING_TYPE = "RSA/ECB/OAEPWITHSHA-1ANDMGF1PADDING";
    ...
    public String payBank(Map<String, String> reqData) throws Exception {
    	
    	String url;
        if (this.useSandbox) {
            url = WXPayConstants.SANDBOX_PAYBANK_URL;
        }
        else {
            url = WXPayConstants.PAYBANK_URL;
        }
        
    	return requestWithCert(url, reqData, config.getHttpConnectTimeoutMs(), this.config.getHttpReadTimeoutMs());
    }
    
    public String payBank(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
    	...
    	return requestWithCert(url, reqData, connectTimeoutMs, readTimeoutMs);
    }
    
    public String payTransfers(Map<String, String> reqData) throws Exception {
    ...
    public String payTransfers(Map<String, String> reqData, int connectTimeoutMs, int readTimeoutMs) throws Exception {
	...
	public String rsaEncrypt(String data) throws Exception {
		...
		if (this.config.getPublicKeyPath() == null || this.config.getPublicKeyPath().trim().length() == 0) {
            throw new Exception("public key path in config is empty");
        }
		
		PublicKey pubKey = RSAUtils.getPubKey(config.getPublicKeyPath(), "RSA");
		byte[] encrypt = RSAUtils.encrypt(data.getBytes(), pubKey, 2048, 11, PADDING_TYPE);
		Encoder encoder = Base64.getEncoder();
		String encodeString = encoder.encodeToString(encrypt);
		return encodeString;
	}
